...
| L-UNBOUND | Projected signatures that are not both `valid` and `bound` **under a grounding contract** attribute no agent; they carry `wrt:claimedSigner`. Scoped to signature nodes that EXIST — a loss describing a node the graph lacks is a caveat on an absent fact |
